...
- [x] **Task 7 — Build, host tests, on-device check (AC1–AC4).** (AC: 1, 2, 3, 4)
  ...
  - [x] On-device (Fenix 8 sideload) — verify **resume-never-lies**: (a) read a few words, pause, **force-quit via the carousel** (navigate away — NOT a graceful BACK exit), relaunch → app opens **Paused at-or-before** the last word seen, **never past it**; (b) read → pause → relaunch lands on the paused word (clean exit path); (c) rewind then relaunch lands at-or-before the rewound word; (d) read to the end (Finished) then relaunch → does not crash, lands sanely (Finished or last word, not past end); (e) playback/paused/context/chapter-card/finished flows from 3.2–3.5 unchanged. Record the result in the sprint note (machine-readable, per repo memory).
  - [ ] (optional, not run) If feasible, eyeball the schema bump: temporarily set `SCHEMA_VERSION_CURRENT = 2` on a device that already has a saved position, relaunch → position MUST survive the bump (only the — currently empty — content cache is wiped). Revert to `1` (do **not** commit the bump).

### Review Findings

Code review 2026-07-14 (3 layers — Blind Hunter / Edge Case Hunter / Acceptance Auditor). All 4 ACs MET (auditor). 4 patch, 8 defer, 3 dismissed.

- [x] [Review][Patch] Empty-decode session force-saves index 0 over a good saved position — `CannedWordSource.decode()` degrades to an empty book (wordCount 0) while `bookId()` still returns the real fingerprint; `onHide`/`onStop`/tick force-commits then write `{"pos"=>0}` under `pos_4bd588b9`, destroying the sacred saved position. Guard the view's helper: skip commits when `_source.wordCount() <= 0` [watch/source/views/PlaybackView.mc:189]
- [x] [Review][Patch] Schema-read-throw is conflated with fresh install and restamped as current — a throwing `Storage.getValue(SCHEMA_VERSION)` sets `stored = null` and falls through to stamping `SCHEMA_VERSION_CURRENT`, silently skipping the mismatch-wipe; abort the migration (return, leave the stamp) on a failed read [watch/source/sync/SyncManager.mc:135]
- [x] [Review][Patch] Failed-write retry storm: persistent Storage failure retries `setValue`+`println` every tick — `_lastWriteMs` advances only on success, so once the window elapses a persistently-throwing store (e.g. StorageFullException) is retried on every ~85 ms tick at high WPM; back off one debounce window on a failed write (advance `_lastWriteMs` in the catch too) [watch/source/sync/SyncManager.mc:116]
- [x] [Review][Patch] No same-value suppression: force saves rewrite identical records to flash — exit writes twice back-to-back (`onHide` then `onStop`), rewind-while-playing writes twice (explicit force + settling tick), every context-view round-trip rewrites the unchanged position; track the last successfully written index in `SyncManager` and skip identical writes [watch/source/sync/SyncManager.mc:116]
- [x] [Review][Defer] Position-record hardening for Epic-4 LWW [watch/source/sync/SyncManager.mc:54-83] — deferred, Epic-4-armed: `decodeIndex` rejects `Long`/`Float` `pos` outright (Comms/JSON numbers routinely arrive as `Long` → a valid phone-written position silently resets to word 0); `ts` is never validated; `ts` epoch-sec in a signed 32-bit `Number` inverts LWW comparisons in Jan 2038; a debounced write stamps `ts` at write time, up to ~15 s after the position was reached (skew can lose a legitimate LWW race). Harden with Story 4.5.
- [x] [Review][Defer] `bookId()` base default `""` collapses identity-less sources onto the single key `"pos_"` [watch/source/engine/BookWordSource.mc:53] — deferred, Epic-4-armed: a `ChunkedWordSource` constructed before/without its manifest returns `""` and cross-reads/clobbers another identity-less book's position. Treat an empty/non-fingerprint `bookId` as no-persistence in `SyncManager` when Epic 4 adds the second source.
- [x] [Review][Defer] Schema downgrade (`stored > SCHEMA_VERSION_CURRENT`) silently restamped down with the older wipe list [watch/source/sync/SyncManager.mc:135] — deferred, Epic-4: an older build over newer storage can't enumerate newer content keys (orphans survive the "wipe") and a newer-layout position record degrades to word 0; guard = leave the stamp, log, skip. Decide downgrade semantics when a v2 layout first exists.
- [x] [Review][Defer] Adapter half untested: migration branch + `_lastWriteMs` invariants [watch/source-test/SyncManagerTest.mc] — deferred: the mismatch-wipe branch has never executed anywhere (the optional on-device schema-bump eyeball was skipped) and "advances only on successful write" is pinned by no test; extract the migration decision as a pure function + host-test it when Story 4.1 fills the wipe list.
- [x] [Review][Defer] `pos_*` records are never garbage-collected [watch/source/sync/SyncManager.mc:135, watch/source/source_data/CannedWordSource.mc:42] — deferred, Epic-4: fingerprint drift on re-bake and book deletion orphan position records forever in a quota-bounded store, and the migration routine is (correctly) forbidden from touching them; eviction belongs to the Epic-4 book-lifecycle/`BookChanged` story.
- [x] [Review][Defer] Kill during a chapter card permanently skips that chapter's card [watch/source/views/PlaybackView.mc:354-374] — deferred: restore lands Paused ON the flagged first word (which the card occluded — strictly one word past "last seen"); resume advances past it so `maybeEnterChapterCard` never re-fires for that chapter. Same family as 3.5's deferred `_cardedIndex`-never-reset; cosmetic (one missed ~2 s card).
- [x] [Review][Defer] Debounced save can persist up to CATCHUP_CAP(=4) words past the last painted word [watch/source/views/PlaybackView.mc:329] — deferred: the commit runs after `_engine.onTick(now)` but before `WatchUi.requestUpdate()`; a kill inside that ms window right after a catch-up burst restores past the last word actually seen. Vanishingly narrow in Epic 3; revisit with Epic-4 buffering stalls (capture the pre-tick index for the debounced write).
- [x] [Review][Defer] Synchronous flash write on the hot tick path [watch/source/views/PlaybackView.mc:329] — deferred: every ~15 s one playing tick pays a synchronous `Storage.setValue`; a slow flash write delays the tick and the accumulator answers with a multi-word catch-up burst — a periodic stutter the simulator cannot reproduce. Measure during Story 3.9 (Gate V4 reading-session hardware gate).
...
- **Task 7** — Host suite 81/81 green @ Strict L3 in the CI image (73 → 81: +5 seekTo, +3 Sync); release + unit-test builds clean/warning-free on both the gate image (8.4.0) and on-host SDK 9.1.0. Signed `watch/bin/PaceTurner.prg` (43,404 B) sideloaded via MTP/gio. **ON-DEVICE PASS 2026-07-02 (Fenix 8, Nerya): all 5 checks green** — (a) carousel force-quit mid-read → relaunch Paused at-or-before last word PASS; (b) pause → exit → relaunch on the paused word PASS; (c) rewind → relaunch at-or-before PASS; (d) read-to-Finished → relaunch sane PASS; (e) 3.2–3.5 flows no-regression PASS. Optional schema-bump eyeball (SCHEMA_VERSION_CURRENT=2 locally) not run — the mismatch path is host-logic-only in Epic 3 (structured no-op wipe) and remains available before Epic 4 fills the wipe list.
...
- 2026-07-02 (Story 3.6, on-device): Fenix 8 sideload — ON-DEVICE PASS, all 5 resume-never-lies checks green (carousel-kill, pause, rewind, finished relaunch, 3.2–3.5 no-regression). Optional schema-bump eyeball not run.
