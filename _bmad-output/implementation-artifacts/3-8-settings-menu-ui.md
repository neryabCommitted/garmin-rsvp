...
- [x] Task 6: NEW WPM stepper editor — `views/WpmStepperView.mc` + `input/WpmStepperDelegate.mc` (AC: 1, 2)
  ...
  - [x] Delegate (`WatchUi.InputDelegate`, raw `onKey` — never `onSelect`, UX-DR15): UP/DOWN = ±adaptive step on the candidate; START = commit (`_settings.wpm = candidate; _settings.save(); view.applyWpm(candidate); menuItem.setSubLabel(...)`) + pop; BACK = cancel + pop (pop-not-exit).
...
- [ ] Task 9: On-device Fenix 8 check (human, results machine-readable to stdout/log)
  - [ ] MENU while playing → pauses at current word + menu opens; MENU while paused → menu opens; MENU during chapter card / FINISHED → nothing.
  - [ ] Each item takes effect immediately on pop-back: touch Off → tap dead but buttons work; handedness Left → rewind swipe mirrors to swipe-left; font size visibly changes the word; anchor 60% visibly shifts the pivot column; focus highlight / phantom toggles visible; pause mode Instant → START freezes without coast; chapter resume Wait → next card holds for START; WPM stepper commit → flow speed changes next word.
  - [ ] Persistence: set non-defaults → carousel force-quit → relaunch → menu shows the saved values, reading uses them (incl. stepped-in-flow WPM if Task 5 decision stands).
  - [ ] Regressions: BACK from menu → Paused frame (not exit); wake-press does NOT open the menu (grace); 3.2–3.7 spot checks (context view, card, resume-never-lies, wake-finds-Paused).
