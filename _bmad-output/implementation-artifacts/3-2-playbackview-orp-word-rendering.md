...
- [x] **Task 6 — Verify (Strict L3 + host tests green + visual on device)**
  ...
  - [~] The View's `onUpdate` drawing is not host-unit-testable (no `Dc` in the harness) — correctness covered by the pure helpers (Task 1, 8 tests) + Strict-L3 compile (both builds). A one-line readiness marker is printed on `onShow` (`System.println`, machine-readable, NOT per-word). **PENDING (human/hardware): the visual check on the real Fenix 8** (sideload per the toolchain memo) — cannot be automated; recommended before Story 3.3 to de-risk. See Completion Notes.
...
- **All 5 ACs implemented; 8 of 9 Task-6 subtasks fully verified.** The one open item is the **on-device visual check** (AC5's "is the composition right on real hardware") — inherently a human/hardware step (sideload to the Fenix 8). It is NOT a code gap: the pure layout math is covered by 8 host tests, both builds compile clean at Strict L3, and the `onShow` readiness marker (`"PlaybackView ready: words=228 wpm=250 fontIdx=0"`) prints for log-based confirmation. **Recommended: sideload + eyeball before Story 3.3** (controls), per the story's own de-risking note. Gate V4 (battery) remains Story 3.9.
...
### Change Log
- 2026-06-22 — Story 3.2 implemented (dev-story): OrpLayout pure helpers + PlaybackView ORP render surface + timer loop over CannedWordSource (base64 dev stream). 5/5 ACs met; resolves 3.1 deferred #1 (fontSize clamp) + #4 (per-duration timer arming). 46/46 host tests green @ Strict L3, release build clean. On-device visual check pending (human/hardware).
